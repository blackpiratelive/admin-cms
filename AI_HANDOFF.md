...
- **Dashboard reads & Server-side List queries:** Keep list and dashboard views bounded. Microblog list queries use server-side pagination (`limit: 50` default), server-side status filtering and search, and select only necessary display columns rather than fetching full records. `getMicroblogDashboardData`, `getRecentMoviesAction`, and `getRecentShowsAction` fetch only display records. Work should be initiated concurrently with `Promise.all`.
