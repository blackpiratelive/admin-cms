...
- `lib/core/network/`: `api_client.dart` (REST client handling dynamic server URL connection, authentication, Microblog CRUD, Journal E2EE CRUD, Encrypted Raw Asset uploads/downloads, location/trip/people pickers, image uploads, and Vercel deployment hook trigger).
...
- `lib/modules/journal/`: `journal_unlock_modal.dart` (Master password unlock & session vault setup dialog), `journal_main_screen.dart` (Flagship vault view with mobile-responsive 2x2 stat cards, Lexical JSON AST text extractor, decrypted fuzzy search, and sub-tabs for Timeline list, Monthly Calendar, and Mood/Streak Stats), `journal_editor_screen.dart` (CRUD entry editor with mobile responsive layout, Lexical JSON builder/parser, formatting toolbar, date/type/mood/template inserter, `JournalAttachmentsWidget` for E2EE blobs, and functional Context & Connections sidebar).
